...
TS <- ggplot(sdr_data, aes(x = goal_3_score, 
                           y = goal_6_score, label = country)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  stat_cor(aes(label = paste("R =", ..r.., ", p =", ..p..)), label.x = 3, label.y = 3) +
  theme_minimal()
...
- Edit the code chunk above to plot a linear regression of 2 different sdg scores 
  ...
  - in 4-6 sentences, come up with a hypotheses for the positive or negative correlation between the 2 goals
      ...
      - are there any points that don't follow the trend (outliers)
        ...
        The graph shows a strong positive correlation between goal_3_score and goal_6_score, with an R-value of 0.76, indicating that as countries score higher on goal 3 (Good Health and Well-Being), they also tend to score higher on goal 6 (Clean Water and Sanitation). The low P score tells me that this correlation did not happen by chance and is indeed linked to one another. I can tell there is a positive correlation between these two goals. Despite the observation, there are a few outliers but no countries deviate too far such as Oceania and the African Republic. However, the higher a country's Good Health and Well-Being score is the higher their Clean Water and Sanitation score is. 