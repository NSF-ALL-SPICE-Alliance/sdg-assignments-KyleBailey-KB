...
```{r}
ggplot(data = sdr_data, aes(x= goal_7_score, y = goal_8_score, color = goal_9_score)) +
  geom_point()
```

It seems like countries with high SDG 9 scores (Industry, Innovation, and Infrastructure) tend to have higher SDG 8 (Decent Work and Economic Growth) scores and SDG 7 (Affordable and Clean Energy) scores.Despite there being a few outliers this does seem to be the trend based on the visual made here.
Challenge Uno 
...
ggplot(MENA_sdr_data, aes(x= goal_3_score, y = country)) +
  geom_bar(stat = "identity") #Challenge Dos
```
...
```
It looks like for SDG 2 (Zero Hunger) that most regions don't pass more than a 70%. Only areas even close to reaching that goal are a few countries within the East & South Asia region and OECD.
Challenge 3
