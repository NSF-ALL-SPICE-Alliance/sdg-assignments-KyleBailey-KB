...
```{r}
k5 <- kmeans(sdr_data_imputed, centers = 5)
#Challenge Uno
```
...
```{r}
fviz_cluster(k5, data = sdr_data_imputed) +
  theme_minimal()
```
...
```{r}
country_clusters <- as.data.frame(k5$cluster)
```
...
```{r}
rf_obesity <- randomForest(normalized_score_sdg2_obesity ~ .,
...
```{r}
pdp::partial(rf_obesity, pred.var = "normalized_score_sdg6_sanita", plot = TRUE)
```
...
```{r}
pd <- pdp::partial(rf_obesity, pred.var = c("normalized_score_sdg6_sanita", "normalized_score_sdg3_u5mort"))
# Default PDP
plotPartial(pd)
```
